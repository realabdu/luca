...
// Fallback data for when Convex is loading or empty
const FALLBACK_EVENTS = [
  { _id: '1' as any, timestamp: Date.now(), timeLabel: 'Just now', amount: 450, source: 'TikTok' as const, campaign: 'Summer_Sale_v2', creativeUrl: 'https://picsum.photos/id/1/200/120', status: 'Paid' as const },
  { _id: '2' as any, timestamp: Date.now() - 4 * 60000, timeLabel: '4m', amount: 120.50, source: 'Google' as const, campaign: '"best perfumes"', creativeUrl: '', status: 'Pending' as const },
  { _id: '3' as any, timestamp: Date.now() - 12 * 60000, timeLabel: '12m', amount: 990, source: 'Snapchat' as const, campaign: 'Influencer_Pack', creativeUrl: 'https://picsum.photos/id/2/200/120', status: 'Paid' as const },
  { _id: '4' as any, timestamp: Date.now() - 15 * 60000, timeLabel: '15m', amount: 230, source: 'Meta' as const, campaign: 'Retargeting_Q3', creativeUrl: 'https://picsum.photos/id/3/200/120', status: 'Paid' as const },
];
...
export default function LiveFeedContent() {
  ...
  const eventsData = useQuery(api.events.list, canQuery ? {
    source: selectedSource ?? undefined,
    limit: 10
  } : "skip");
  const recentStats = useQuery(api.events.getRecentStats, canQuery ? {} : "skip");
      ...
      <div className="flex-1 flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-4 relative">
          ...
          {events.map((event, index) => (
            <div key={event._id} className={`group relative flex gap-4 md:gap-6 items-start ${index > 5 ? 'opacity-50' : 'opacity-100'}`}>
              ...
              <div className="flex-1 bg-white p-5 border border-border-light shadow-sm cursor-pointer">
                <div className="flex flex-wrap justify-between items-start mb-4 gap-4">
                  <div className="flex items-center gap-3">
                    <span className="bg-primary/10 text-primary px-2.5 py-1 text-xs font-bold">#{event._id.toString().slice(0, 4)}</span>
