...
// Import api with fallback for when integrations module doesn't exist yet
...
export default function IntegrationsContent() {
  ...
  // Fetch integrations from Convex (with fallback when integrations module not available)
  ...
  // Disconnect mutation (with fallback)
  const hasDisconnect = api?.integrations?.disconnect;
  const disconnectMutation = useMutation(
    hasDisconnect ? api.integrations.disconnect : undefined
  );

  // Onboarding status mutation (with fallback)
  const hasOnboardingRefresh = api?.onboarding?.refreshStatus;
  const refreshOnboardingStatusMutation = useMutation(
    hasOnboardingRefresh ? api.onboarding.refreshStatus : undefined
  );

  const disconnect = async (args: { platform: IntegrationPlatform }) => {
    if (hasDisconnect) {
      return disconnectMutation({ ...args, clerkOrgId });
    }
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      // Update onboarding status after disconnect
      if (hasOnboardingRefresh) {
        await refreshOnboardingStatusMutation({ clerkOrgId });
      }
      setNotification({
        type: "success",
  ...
  return (
    <div className="p-8 max-w-[1200px] mx-auto space-y-10 animate-in fade-in duration-500">
            ...
            return (
              ...
              >
                ...
                <p className="text-sm text-text-muted mb-6 leading-relaxed">
                   ...
                   ) : status?.isConnected ? `Connected to store: ${status.accountName}` : display.description}
                </p>
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-text-muted h-8 mb-4 line-clamp-2">
                   ...
                   ) : status?.isConnected ? `Connected: ${status.accountName}` : display.description}
                </p>
      ...
      {integrations && integrations.some((i) => i.isConnected) && (
                    ...
                    <tr key={integration._id}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-semibold text-text-main text-sm">
                            {PLATFORM_DISPLAY[integration.platform].name}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-sm text-text-muted">
                        {integration.lastSyncAt
                          ? new Date(integration.lastSyncAt).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })
                          : "Pending..."}
                      </td>
                       <td className="px-6 py-4">
                        {integration.tokenExpired ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold bg-red-50 text-red-600 border border-red-100">
                            Re-connect
                          </span>
                        ) : (
